...
if ($datafn=='all'){ 
	...
	if ($rs){
		...
		while($data = mysqli_fetch_array($rs,MYSQLI_BOTH)) {
				...
				$datareturn.= "<ROW>";
				$datareturn.= "		<id>$data[id]</id>";
				$datareturn.= "		<subject>$data[subjectx]</subject>";
				$datareturn.= "		<description>$data[description]</description>";
				$datareturn.= "		<startdate>$data[eventstarts]</startdate>";
				$datareturn.= "		<enddate>$data[eventends]</enddate>";
				$datareturn.= "		<color>$eventcolor</color>";
				$datareturn.= "		<finished>0</finished>";
				$datareturn.= "		<parent>0</parent>";
				$datareturn.= "		<priority>$data[priority]</priority>";
				$datareturn.= "</ROW>
				";		
		}
...
<DATASET>
<totalrowsx>$num_rows</totalrowsx>
$datareturn
</DATASET>";	
	} else { 
		echo ''; 
